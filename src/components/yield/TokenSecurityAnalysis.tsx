...
import { Shield, AlertTriangle, AlertCircle, CheckCircle, Globe, ExternalLink, FileCheck, UserCheck } from 'lucide-react';
import { Alert, AlertDescription } from '@/components/ui/alert';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { getRiskColor, getRiskIcon, formatAddress, openExternalLink } from './utils/tokenDiscoveryUtils';
...
const TokenSecurityAnalysis = ({ arenaResult }: TokenSecurityAnalysisProps) => {
  ...
  const RiskIcon = getRiskIcon(arenaResult.securityRisk) === 'Shield' ? Shield :
                   getRiskIcon(arenaResult.securityRisk) === 'AlertTriangle' ? AlertTriangle : AlertCircle;
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Card className={`bg-background/30 border ${getRiskColor(arenaResult.securityRisk)}`}>
            ...
            <CardContent>
              <div className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${getRiskColor(arenaResult.securityRisk)}`}>
