...
export class EnhancedDexScreenerService {
  ...
  async searchTokenPairs(tokenAddress: string, targetDex?: string) {
    this.debugLogger.log('DEXSCREENER', 'SEARCH_START', { tokenAddress, targetDex })
    ...
    try {
      ...
      if (!data.pairs || data.pairs.length === 0) {
        this.debugLogger.log('DEXSCREENER', 'NO_PAIRS_FOUND', { tokenAddress })
        return null
      }
      ...
      this.debugLogger.log('DEXSCREENER', 'AVALANCHE_PAIRS_FOUND', {
      ...
      if (targetDex) {
        ...
        this.debugLogger.log('DEXSCREENER', 'DEX_FILTERED', {
          targetDex,
      ...
      if (filteredPairs.length === 0) {
        this.debugLogger.log('DEXSCREENER', 'NO_MATCHING_DEX_PAIRS', { targetDex })
        return null
      }
      ...
      this.debugLogger.log('DEXSCREENER', 'BEST_PAIR_SELECTED', result)
      return result
      
    } catch (error) {
      this.debugLogger.log('DEXSCREENER', 'API_ERROR', { error: error.message, tokenAddress })
      return null
    }
  }
  ...
  async getAVAXPrice(): Promise<number> {
    try {
      ...
      if (pairData && pairData.priceUsd > 0) {
        this.debugLogger.log('DEXSCREENER', 'AVAX_PRICE_SUCCESS', { price: pairData.priceUsd })
        return pairData.priceUsd
      }
      
      this.debugLogger.log('DEXSCREENER', 'AVAX_PRICE_FALLBACK', { fallbackPrice: 35 })
      return 35
    } catch (error) {
      this.debugLogger.log('DEXSCREENER', 'AVAX_PRICE_ERROR', { error: error.message })
      return 35
    }
  }
}
